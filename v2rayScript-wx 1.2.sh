...
function install_v2ray(){
 ...
 #nginx站点配置文件的修改
    ...
    setone="http_upgrade"
    settwo="host"
    setthree="remote_addr"
    setfour="proxy_add_x_forwarded_for"
    ...
    server {
      ...
      server_name           ${yourdom};
        location ${v2ray_path} { # 与 V2Ray 配置中的 path 保持一致
           if (${pone}${setone} != "websocket") { # WebSocket协商失败时返回404
           return 404;
           }
           proxy_redirect off;
           proxy_pass http://127.0.0.1:1080; # 假设WebSocket监听在环回地址的10000端口上
           proxy_http_version 1.1;
           proxy_set_header Upgrade ${pone}${setone};
           proxy_set_header Connection "upgrade";
           proxy_set_header Host ${ptwo}${settwo};
           # Show real IP in v2ray access.log
           proxy_set_header X-Real-IP ${pthree}${setthree};
           proxy_set_header X-Forwarded-For ${pfour}${setfour};
        }
    }
EOF
